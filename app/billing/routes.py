...
def get_active_subscription_for_user(
    ...
    This answers:
        "Does this user have paid access right now?"

    It deliberately does not assume that the most recently
    created subscription is necessarily the active one.
    """
...
def get_latest_subscription_for_user(
    ...
    """
    Return the most recently created subscription record.

    This is useful for billing display/history when there is
...
def get_reusable_subscription_for_user(
    user_id: str,
    tenant_id: int,
) -> Subscription | None:
    """
    Return the local subscription that should receive the
    next payment.

    Canceled subscriptions are intentionally excluded.
    """
    return (
...
def index():
    ...
    if current_user.is_admin():
        ...
        if settings_form.validate_on_submit():
            ...
            webhook_token_input = (
                settings_form.webhook_auth_token.data
                or ""
            ).strip()
        ...
        if (
            billing_account
            and not settings_form.is_submitted()
        ):
            settings_form.provider.data = (
                billing_account.provider
            )
        ...
        # This ordering is only for displaying subscription
        # records in the admin billing interface.
        subscriptions = (
            db_session.query(Subscription)
            .filter_by(
                tenant_id=tenant.id
    ...
    if not current_user.is_teacher():
        raise Forbidden(
            "You are not allowed to access billing settings."
        )
    ...
    # This ordering is only for displaying subscription
    # records in the teacher billing interface.
    subscriptions = (
        db_session.query(Subscription)
        .filter_by(
            tenant_id=tenant.id
...
def create_plan():
    ...
    if current_user.is_admin():
        ...
        plan = Plan(
            ...
            amount_cents=(
                int(
                    plan_form.amount_reais.data
                )
                * 100
            ),
            currency=(
                plan_form.currency.data
            ),
            interval=(
                plan_form.interval.data
            ),
            active=(
                plan_form.active.data
            ),

            # Platform plans are universally
            # available to eligible unassigned
            # students.
    ...
    if (
        ...
        flash(
            (
                "Select at least one student or mark "
                "the plan as available to all students."
            ),
            "danger",
        )
    ...
    submitted_student_ids = set(
        plan_form.eligible_student_ids.data
        or []
    )

    if not submitted_student_ids.issubset(
        allowed_student_ids
    ):
        flash(
...
def student_subscription():
    ...
    # Prefer a genuinely active subscription.
    # If there is none, fall back to the latest
    # subscription so the billing page can still
    # show historical/current billing information.
    subscription = (
        get_active_subscription_for_user(
            current_user.id
        )
        or get_latest_subscription_for_user(
            current_user.id
        )
    )
    ...
    # Payment shown on this page should belong to the
    # subscription being displayed.
    latest_payment = None
...
def create_student_subscription():
    ...
    if not form.validate_on_submit():
        print(
            "DEBUG subscription form.errors:",
            form.errors,
        )

        print(
            "DEBUG subscription form.data:",
            form.data,
        )
    ...
    if current_user.assigned_teacher_id:
        ...
        # Platform plans are available to all
        # unassigned internal students.
        plan = (
            db_session.query(Plan)
            .filter(
                Plan.id == plan_id,
    ...
    # --------------------------------------------------
    # Do not generate another PIX if paid access is
    # currently active.
    # --------------------------------------------------
    active_subscription = (
        get_active_subscription_for_user(
    ...
    if active_subscription is not None:
        flash(
            (
                "Sua assinatura ainda está ativa. "
                "Uma nova cobrança poderá ser gerada "
    ...
    # --------------------------------------------------
    # Reuse the existing local subscription whenever
    # possible. Payments represent billing periods;
    # subscriptions represent the ongoing relationship.
    # --------------------------------------------------
    subscription = (
        get_reusable_subscription_for_user(
            current_user.id,
            tenant.id,
        )
    )

    # If there is an existing subscription, prevent
    # duplicate PIX charges while its latest PIX is
    ...
    if subscription is not None:
        ...
        payment_is_pending_and_valid = (
            latest_existing_payment
            and latest_existing_payment.status
            in [
                "pending",
                "awaiting_payment",
            ]
        ...
        if payment_is_pending_and_valid:
            flash(
                (
                    "Você já possui uma cobrança "
                    "PIX em andamento. Use o pagamento "
                    "atual antes de gerar uma nova "
                    "cobrança."
                ),
                "warning",
            )
    ...
    try:
        ...
        asaas_payment = (
            create_pix_payment(
                ...
                description=(
                    f"Assinatura {plan.name}"
                ),
            )
        )

        # --------------------------------------------------
        # First purchase: create the local subscription.
        #
        # Renewal: reuse the existing subscription.
        # --------------------------------------------------
        if subscription is None:
            subscription = Subscription(
                tenant_id=plan.tenant_id,
            ...
            # This is a renewal/new billing period.
            #
            # If the user selects another eligible plan
            # when renewing, the ongoing subscription
            # moves to that plan.
            ...
            # We no longer maintain recurring
            # subscriptions inside Asaas.
            subscription.provider_subscription_id = (
                None
            )

            # Payment confirmation will activate it.
            subscription.status = (
                "inactive"
            )
        ...
        print(
            (
                "DEBUG create_student_subscription "
                "unexpected error:"
            ),
            exc,
        )
...
def asaas_webhook():
    ...
    if event == "PAYMENT_CREATED":
        # Optional:
        # update local payment status to pending.
        return jsonify(
            {"ok": True}
        ), 200
    ...
    # Kept as a legacy fallback for older charges
    # created through Asaas recurring subscriptions.
    provider_subscription_id = (
        payment_data.get(
            "subscription"
        )
        or subscription_data.get(
            "id"
        )
    )
    ...
    try:
        ...
        # New architecture:
        # standalone PIX payments are matched directly
        # through provider_payment_id.
        ...
        # Legacy fallback for charges created before
        # recurring Asaas subscriptions were removed.
        if (
            payment is None
        ...
        if (
            ...
            payment_status
            in OPEN_PAYMENT_STATUSES
        ):
            if (
                subscription is not None
                and not subscription.status
            ):
                subscription.status = (
                    "pending"
                )
            ...
            payment_status
            in PROBLEM_PAYMENT_STATUSES
        ):
            # Conservative first version:
            # do not deactivate the user automatically
            # based only on a failed/expired charge.
            #
            # Access expiration is determined by
            # current_period_end through
            # sync_internal_access().
            if subscription is not None:
                subscription.status = (
                    payment_status
                )
